...
export async function initDb() {
  ...
  while (retries > 0) {
    try {
      ...
      console.error(`Error initializing database (Attempt ${4 - retries}):`, err);
      retries--;
      if (retries > 0) {
        console.log('Retrying in 2 seconds...');
        await new Promise(resolve => setTimeout(resolve, 2000));
      } else {
        console.error('All database initialization attempts failed. Please check your DATABASE_URL and network connection.');
      }
    } finally {
      if (client) {
        client.release();
